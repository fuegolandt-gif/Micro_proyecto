...
// ==============================================================================
// FUNCIÓN DEL SENSOR DE TEMPERATURA (DHT11 - ALTA SENSIBILIDAD)
// ==============================================================================
float leer_temperatura_dht11() {
    ...
    // Verificar Checksum y procesar la parte decimal
...
void app_main(void) {
    ...
    while (1) {
        ...
        // --- B. Lógica de Control de Temperatura (Con Histéresis y Decimales) ---
        if (temp != -999.0) { 
            if (temp > 7.0) {
                gpio_set_level(PELTIER_PIN, 1); // Enciende MOSFET
                printf("Temp: %.1f C -> [ALERTA] Umbral superado (>7.0C). Enfriamiento ENCENDIDO\n", temp);
            } else if (temp <= 5.0) {
                gpio_set_level(PELTIER_PIN, 0); // Apaga MOSFET
                printf("Temp: %.1f C -> Temperatura ideal alcanzada (<=5.0C). Enfriamiento APAGADO\n", temp);
            } else {
                // Zona muerta (entre 5.1C y 7.0C): mantiene el estado actual
        ...
        // --- D. Esperar 10 segundos antes del siguiente ciclo ---
        vTaskDelay(100 / portTICK_PERIOD_MS); 
    }
}